                                    ...
                                    <tbody class="border-0">
                                        @if ($jobs->isNotEmpty())
                                            @foreach ($jobs as $job)
                                                <tr class="active">
                                                    <td>
                                                        <div class="job-name fw-500">{{ $job->jobPost->title }}</div>
                                                        <div class="info1">{{ $job->jobPost->jobType->job_type }} .
                                                    ...
                                                    <td>
                                                        <div class="job-status text-capitalize">
                                                            {{ $job->jobPost->status == 1 ? 'active' : 'expired' }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="action-dots float-end">
                                                            ...
                                                            <ul class="dropdown-menu dropdown-menu-end">
                                                                ...
                                                                {{-- <li><a class="dropdown-item"
                                                                        href="{{ route('job.edit', base64_encode($job->jobPost->id)) }}"><i
    ...
    <script>
        function deleteJob(id) {
            if (confirm("Are you sure you want to remove this job?")) {
                $.ajax({
                    url: "{{ route('job.removeAppliedJobs') }}",
                    method: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id
